    ...
    class Transform:
        ...
        def Mod_BWHours_Period(self):
            ...
            ETL.val_TYHours_1['Actual time'] = (ETL.val_TYHours_1['Actual time'].replace('[)]', '', regex=True).replace(
                '[(]', '-', regex=True).replace('[,]', '', regex=True).astype(float))
            ETL.val_TYHours['Debited acty type'] = (ETL.val_TYHours['Debited acty type'].
                                                    replace('[#]', '0', regex=True).astype(int))
            ETL.val_TYHours_1['Debited acty type'] = (ETL.val_TYHours_1['Debited acty type'].
                                                    replace('[#]', '0', regex=True).astype(int))
        ...
        def Mod_BWHours_Weekly(self):
            ...
            ETL.val_LYHours_1['Actual time'] = (ETL.val_LYHours_1['Actual time'].replace('[)]', '', regex=True).replace(
                '[(]', '-', regex=True).replace('[,]', '', regex=True).astype(float))
            ETL.val_LYHours['Debited acty type'] = (ETL.val_LYHours['Debited acty type'].
                                                    replace('[#]', '0', regex=True).astype(int))
            ETL.val_LYHours_1['Debited acty type'] = (ETL.val_LYHours_1['Debited acty type'].
                                                      replace('[#]', '0', regex=True).astype(int))
